...
fun main() {
    val program = "2,4,1,1,7,5,1,4,0,3,4,5,5,5,3,0".split(",").filter { it.isNotBlank() }.map { it.toLong() }
...
fun combo(operador:Long, a:Long, b:Long, c:Long) : Long {
    return when (operador) {
        0L,1L,2L,3L -> operador
        4L -> a
        5L -> b
        6L -> c
        else -> 0
